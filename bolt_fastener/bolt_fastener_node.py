...
from armstrong_py.conversions import (list_to_ros_point, list_to_ros_vec3,
                                      ros_pos_to_np_pos, ros_quat_to_rotation,
                                      rot_to_ros_quat, transform_to_pose)
from ass_msgs.msg import ARMstrongPlanRequest, ARMstrongPlanStatus, DXLCommand
...
class BoltFastenerNode(Node):
    ...
    def _initialize_state_variables(self):
        ...
        self.insert_translation = [
            ...
            0.0,
            0.0,
        ]  # Initialize insert translation vector
        ...
        self.head_confidence_threshold = 0.0
        ...
        # Hand camera state
        self.hand_bolts = []
        self.hand_confidence_threshold = 0.0
    ...
    def _setup_camera_transforms(self, tf_static_broadcaster):
        ...
        head_cam_transform.transform.translation = list_to_ros_vec3(
            np.array([-75.31, 0 + 47.5, 531.14]) / 1000
        )
        head_cam_transform.transform.rotation = rot_to_ros_quat(
            Rotation.from_rotvec([0, 30 * np.pi / 180, 0])
        )
        ...
        hand_cam_transform.transform.translation = list_to_ros_vec3(
            np.array([228.01, -35.5 - 17.5, 46.0]) / 1000
        )
    ...
    def _setup_head_camera_subscribers(self):
        ...
        self.rgb_sub = self.create_subscription(
            ...
            topic=self.tcp_tunnel_prefix
            + self.camera_namespace
            + self.head_camera_name
    ...
    def _setup_clients(self):
        ...
        def get_param():
            res = subprocess.run(
                ["ros2", "param", "get", "backass", "offset.R6"],
                capture_output=True,
                text=True,
            )
    ...
    def _setup_loops(self):
        ...
        # Head camera timer
        self.head_draw_timer = self.create_timer(
            1 / 30, self.on_head_draw_timer, callback_group=self.reentrant_group
        )
        ...
        # Hand camera timer
        self.hand_draw_timer = self.create_timer(
            1 / 30, self.on_hand_draw_timer, callback_group=self.reentrant_group
        )
        ...
        self.control_timer = self.create_timer(
            1 / 5, self.on_control_loop, callback_group=self.mutex_group
        )
    ...
    def _setup_status_maps(self):
        ...
        M32_insert_status = InsertStatus(
            ...
            fasten=(0, 0, 0),
            retract=(-0.035, 0, 0),
            full_retract=tuple(np.deg2rad((-25.0, 0.0, 0.0, 25.0, 0.0, 50.0))),
        )
    ...
    def on_head_image_update(self, msg: CompressedImage):
        ...
        try:
            ...
            try:
                ...
                self.head_confidence_threshold = (
                    cv2.getTrackbarPos("Confidence", "head_rgb") / 100.0
                )
            ...
            try:
                str_msg = msg.data
                buf = np.ndarray(
                    shape=(1, len(str_msg)), dtype=np.uint8, buffer=msg.data
                )
                ...
                img = cv2.undistort(
                    img,
                    self.head_rgb_intrinsics["mtx"],
                    self.head_rgb_intrinsics["dist"],
                )
                # Apply contrast and brightness
                img = cv2.convertScaleAbs(
                    img, alpha=(contrast - 50.0) / 50 + 1, beta=brightness - 50
                )
                self.head_rgb = img
                ...
                self.get_logger().error(
                    f"Unexpected error processing head image: {str(e)}"
                )
        except Exception as e:
            self.get_logger().error(
                f"Critical error in head image processing: {str(e)}"
            )
    ...
    def on_hand_image_update(self, msg: CompressedImage):
        ...
        self.hand_confidence_threshold = (
            cv2.getTrackbarPos("Confidence", "hand_rgb") / 100.0
        )
    ...
    def on_head_depth_update(self, msg: CompressedImage):
        ...
        try:
            ...
            try:
                ...
                depth = cv2.undistort(
                    depth,
                    self.head_depth_intrinsics["mtx"],
                    self.head_depth_intrinsics["dist"],
                )
                ...
                self.get_logger().error(
                    f"OpenCV error processing depth image: {str(e)}"
                )
                return
            except Exception as e:
            ...
            if (
                ...
                try:
                    ...
                    self.get_logger().error(
                        f"Value error in bolt pose estimation: {str(e)}"
                    )
                except IndexError as e:
                    self.get_logger().error(
                        f"Index error in bolt pose estimation: {str(e)}"
                    )
                except Exception as e:
            ...
            self.get_logger().error(
                f"Critical error in head depth processing: {str(e)}"
            )
    ...
    def on_hand_depth_update(self, msg: CompressedImage):
        ...
        try:
            ...
            try:
                ...
                self.get_logger().error(
                    f"Index error decoding hand depth image: {str(e)}"
                )
                return
            except cv2.error as e:
                self.get_logger().error(
                    f"OpenCV error processing hand depth image: {str(e)}"
                )
                return
            except Exception as e:
            ...
            self.get_logger().error(
                f"Critical error in hand depth processing: {str(e)}"
            )
    ...
    def on_hand_imu_update(self, msg: Imu):
        ...
        self.imu_ori = Rotation.from_quat(
    ...
    def on_armstrong_plan_status_update(self, msg: ARMstrongPlanStatus):
        ...
        if self.armstrong_plan_status == "PLANNED" and self.status == "RETRYING":
            self.retry_count = 0
        if (
            self.armstrong_plan_status == ARMstrongPlanStatus.COMPLETED
            and self.status == "ALIGNING"
        ):
            self.status = "ALIGNED"
    ...
    def retry_align(self):
        ...
        tgt_rot = ros_quat_to_rotation(self.target_pose.orientation).as_euler(
            "xyz", degrees=True
        )
        tgt_rot[0] = tgt_roll
        self.target_pose.orientation = rot_to_ros_quat(
            Rotation.from_euler("xyz", tgt_rot, degrees=True)
        )
        self.follow_target(self.target_pose)
        self.retry_count += 1

    def sine_sweep(self, initial_pose):
        pass

    def on_head_mouse_event(
        self, event: int, x: int, y: int, flags: int, params=None
    ) -> None:
            ...
            for i, (x1, y1, x2, y2) in enumerate(self.head_bolts_detection_info.xyxy):
                if x1 <= x <= x2 and y1 <= y <= y2:
                    ...
                    self.target_pose = self.get_target_pose(
                        target, tgt_offset=[-0.40, 0, -0.05]
                    )
                    if self.target_pose is not None:
                        self.follow_target(self.target_pose)
                    return

    def on_hand_mouse_event(
        self, event: int, x: int, y: int, flags: int, params=None
    ) -> None:
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            for i, (x1, y1, x2, y2) in enumerate(self.hand_detection.xyxy):
                if x1 <= x <= x2 and y1 <= y <= y2:
                    self.target = self.make_bolt_name(
                        self.hand_detection.box_id[i], "", "hand"
                    )
    ...
    def on_head_draw_timer(self) -> None:
        ...
        if self.head_bolts_detection_info is not None:
            rgb = self.head_detector.draw(
                rgb, self.head_bolts_detection_info, self.head_bolts
            )
    ...
    def on_hand_draw_timer(self):
        ...
        if (
            ...
            and self.colored_mask is not None
            and self.status == "DOCKING"
        ):
    ...
    def _handle_docking_control_by_sam(self):
        ...
        if (
            ...
            if len(target_indices) > 0:
                ...
                seg = self.sam.predict(
                    self.hand_rgb, points=[target_point], verbose=False
                )[0]
                mask = (
                    seg.masks.data.cpu()
                    .numpy()
                    .reshape(
                        self.hand_rgb_intrinsics["height"],
                        self.hand_rgb_intrinsics["width"],
                    )
                )
                colored_mask = cv2.cvtColor(mask.astype("uint8"), cv2.COLOR_GRAY2BGR)
                self.colored_mask = (colored_mask * np.array((0, 0, 255))).astype(
                    "uint8"
                )
                ...
                center_px = (
                    self.hand_rgb_intrinsics["width"] // 2,
                    self.hand_rgb_intrinsics["height"] // 2,
                )
                box = np.array(
                    [
                        center_px[0] - self.tolerance,
                        center_px[1] - self.tolerance,
                        center_px[0] + self.tolerance,
                        center_px[1] + self.tolerance,
                    ]
                ).astype(np.int32)
    ...
    def _handle_inserting_control(self):
        ...
        if self.armstrong_plan_status not in [
            ARMstrongPlanStatus.IDLE,
            ARMstrongPlanStatus.COMPLETED,
            ARMstrongPlanStatus.FAILED,
        ]:
            return
        ...
        self.perform_inserting(
            list(self.insert_status.get_current_params()),
            trigger,
            vel_scale=vel_scale,
    ...
    def _setup_projected_bolt_box(self):
        ...
        if self.projected_bolt_box_area is None:
            ...
            self.projected_bolt_box = self.projected_bolt_box.reshape(-1, 2).astype(
                np.int32
            )
            self.projected_bolt_box_area = np.multiply(
                *(self.projected_bolt_box[0] - self.projected_bolt_box[1])
            )
            ...
            self.projected_bolt_circle = self.projected_bolt_circle.reshape(
                -1, 2
            ).astype(np.int32)
            self.projected_bolt_circle_area = (
                np.sum(
                    np.square(
                        (self.projected_bolt_circle[0] - self.projected_bolt_circle[1])
                    )
                )
                * np.pi
            )
    ...
    def _draw_target_boxes(self, rgb):
        ...
        rgb = cv2.rectangle(
            rgb,
            self.projected_bolt_box[0],
            self.projected_bolt_box[1],
            (255, 255, 0),
            2,
        )
    ...
    def _draw_status_info(self, rgb):
        ...
        rgb = cv2.putText(
            rgb,
            f"Detection: {'ON' if self.is_head_update_detection else 'OFF'}",
            (10, y_offset),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (0, 255, 0) if self.is_head_update_detection else (0, 0, 255),
            2,
        )
        y_offset += line_height
    ...
    def _set_offset_by_imu(self):
        ...
        p, y, r = self.imu_ori.as_euler(
            "xyz", degrees=True
        )  # as imu's axis differs with robot (X: Right, Y: Down, Z: Forward)
        p += 90
    ...
    def set_target(
        self, box_id, group_id, prefix: Literal["head", "hand"] = "head"
    ) -> str:
    ...
    def perform_docking_by_box_sam(self, tol_box, target_bbox_area, target_point):
        ...
        cam_z = self.get_abstract_depth_from_box(
            self.bolt_radius**2 * np.pi,
            target_bbox_area,
            self.hand_rgb_intrinsics["fx"],
            self.hand_rgb_intrinsics["fy"],
        )
        cam_x, cam_y, cam_z = self.pixel_to_camera_coords(
            t_x,
            t_y,
            cam_z,
            self.hand_rgb_intrinsics["fx"],
            self.hand_rgb_intrinsics["fy"],
            self.hand_rgb_intrinsics["cx"],
            self.hand_rgb_intrinsics["cy"],
        )
        ...
        self.get_logger().info(
            f"Move step: {x_move_step}, {y_move_step}, {z_move_step}"
        )
    ...
    def apply_rotation_to_transform(
        self, source_pose: Pose | Transform, target_tls: np.ndarray
    ) -> Pose:
    ...
    def perform_inserting(
        self,
        target: List,
        target_trigger: int = 512,
        vel_scale: float = 0.005,
        wait_after_complete: float = 0.5,
    ) -> None:
        """Perform inserting operation"""
        assert self.insert_pose is not None
        if len(target) == 3:
            self.insert_pose = self.apply_rotation_to_transform(
                self.insert_pose, np.array(target)
            )
            msg = self.build_armstrong_plan_request(
                group_name="right_arm",
                link_name="r_link6",
    ...
    def perform_transform(
        self,
        source_pose: Tuple[np.ndarray, Rotation],
        target_transform: Tuple[np.ndarray, Rotation],
    ):
    ...
    def publish_transform(
        self, target: Tuple[str, np.ndarray, Rotation], source_frame: str
    ) -> None:
    ...
    def follow_target(self, target_pose: Pose) -> None:
        ...
        if self.armstrong_plan_status in [
            ARMstrongPlanStatus.IDLE,
            ARMstrongPlanStatus.COMPLETED,
            ARMstrongPlanStatus.FAILED,
        ]:
    ...
    def make_bolt_name(
        ...
        return "-".join(
            map(
                str,
                [prefix, group_id, box_id, "bolt"]
                if group_id is not None
                else ["unknown", box_id, "bolt"],
            )
        )
    ...
    def toggle_camera_location(
        self, location: Optional[Literal["head", "hand"]] = None
    ):
    ...
    def build_armstrong_plan_request(
        ...
        assert target_pose or target_transform or target_joint, (
            "At least one of target_pose, target_transform, or target_joint must be provided."
        )
